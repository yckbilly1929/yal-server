...
function validateBinaryVersion(...command: string[]): void {
  ...
  const stdout = child_process
    ...
    .toString()
    .trim()
  if (stdout !== YALIVE_VERSION) {
    throw new Error(`Expected ${JSON.stringify(YALIVE_VERSION)} but got ${JSON.stringify(stdout)}`)
  }
}
...
function installUsingNPM(pkg: string, subpath: string, binPath: string): void {
  ...
  // Create a temporary directory inside the "yalive-server" package with an empty
  // "package.json" file. We'll use this to run "npm install" in.
  const yaliveLibDir = path.dirname(require.resolve('yalive-server'))
  const installDir = path.join(yaliveLibDir, 'npm-install')
...
function applyManualBinaryPathOverride(overridePath: string): void {
  // Patch the CLI use case (the "yalive-server" command)
...
async function downloadDirectlyFromNPM(pkg: string, subpath: string, binPath: string): Promise<void> {
  ...
  const url = `https://registry.npmjs.org/${pkg}/-/${pkg}-${YALIVE_VERSION}.tgz`
...
`)
    ...
    // to be helpful but it's not the supported way of installing yalive-server.
